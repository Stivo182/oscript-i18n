...
Основой библиотеки служат пакеты ресурсов, в которых хранятся локализованные ресурсы. Каждый пакет может состоять из нескольких файлов ресурсов. 
Ресурсы представляют из себя переводы текстов, шаблоны, форматы, настройки и прочее. Они описываются в текстовом формате [.properties](https://ru.wikipedia.org/wiki/.properties) в виде пар "ключ-значение", либо через собственный класс `.os`.
...
Более детально описано в описании [публичного интерфейса](docs/README.md)
