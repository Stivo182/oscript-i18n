...
// Прочитанные пакеты ресурсов локализации
//
...
Функция ПолучитьПакет(БазовоеИмя, Знач КодЛокализации = Неопределено) Экспорт
	...
	КодЛокализацииПоУмолчанию = РегиональныеНастройки.ЛокальПоУмолчанию();
	
	ПросканироватьКаталоги();
	
	Если КодЛокализации = Неопределено Тогда
		КодЛокализации = РегиональныеНастройки.ТекущаяЛокаль();
	КонецЕсли;
	...
	Если Пакет = Неопределено Тогда
		ВызватьИсключение СтрШаблон("Не найден пакет ресурсов для указанного базового имени '%1'", БазовоеИмя);
	КонецЕсли;
...
Функция ПолучитьПакеты(БазовыеИмена, Знач КодЛокализации = Неопределено) Экспорт
	...
	МассивИмен = СтрРазделить(БазовыеИмена, ",");

	ГруппаПакетов = Новый ГруппаПакетовРесурсовЛокализации();

	Для Каждого БазовоеИмя Из МассивИмен Цикл
		ГруппаПакетов.Добавить(ПолучитьПакет(СокрЛП(БазовоеИмя), КодЛокализации));
	КонецЦикла;
...
Функция НайтиПакет(БазовоеИмя, КодЛокализации)
	...
	Для Каждого ИмяПакета Из ВозможныеПакеты Цикл
		...
		Иначе			
			...
			Пакет = Новый ПакетРесурсовЛокализации(БазовоеИмя, КодЛокализации, РесурсыПакета);
...
Функция ПолучитьРесурсыПакетаИзКласса(ИмяПакета)
	...
	ОбъектРесурсовПакета = Неопределено;
	...
	Попытка
		Выполнить(СтрШаблон("ОбъектРесурсовПакета = Новый %1()", ИмяПакета));
		ОбъектРесурсовПакета = ОбъектРесурсовПакета.ПолучитьРесурсы();
	Исключение
		Возврат Неопределено;
	КонецПопытки;

	Если Не ТипЗнч(ОбъектРесурсовПакета) = Тип("Соответствие") Тогда
		Возврат Неопределено;
	КонецЕсли;

	Возврат ОбъектРесурсовПакета;
